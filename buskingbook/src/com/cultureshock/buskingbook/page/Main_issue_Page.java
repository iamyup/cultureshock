...
public class Main_issue_Page extends LinearLayout implements View.OnClickListener, HttpClientNet.OnResponseListener{
...
//		dialog.setCanceledOnTouchOutside(false);
	...
	public void setui()
	{
		...
		mLikeImg = (ImageView) v.findViewById(R.id.item_lineup_bottom_likecount_img);
		mLikeCount = (TextView) v.findViewById(R.id.item_lineup_bottom_likecount);
		...
		mLikeImg2 = (ImageView) v.findViewById(R.id.item_lineup_bottom_likecount_img_3);
		mLikeCount2 = (TextView) v.findViewById(R.id.item_lineup_bottom_likecount_3);
...
//    			"    자유와 로맨틱  ");
	 ...
	 public void dataUiset()
	    {
			...
			{
				if(LoginInfoObject.getInstance().getLikeTeamList().get(i).equals(bestTeam.getTeamName()))
				{
	    	...
	    	for(int i = 0 ; i< LoginInfoObject.getInstance().getLikeTeamList().size() ; i++)
			{
				if(LoginInfoObject.getInstance().getLikeTeamList().get(i).equals(issueTeam.getTeamName()))
				{
					mLikeImg2.setBackgroundResource(R.drawable.heart_o);
					break;
				}
			}
	    	m_oAsyncImageLoader.setImageDrawableAsync(mImg2,issueTeam.getTeamThum(),default1,default1,mContext);
		...
		switch(v.getId())
		{
			...
			case R.id.like_select:
			{
				...
				if(LoginInfoObject.getInstance().isLogin())
				{
					 ...
					 {
							if(bestTeam.getLikeMans().get(i).equals(LoginInfoObject.getInstance().getId()))
							{
			...
			case R.id.like_select_3:
			{
				...
				if(LoginInfoObject.getInstance().isLogin())
				{
					 ...
					 {
							if(issueTeam.getLikeMans().get(i).equals(LoginInfoObject.getInstance().getId()))
							{
								Toast.makeText(mContext, "이미 좋아하는 팀입니다.", Toast.LENGTH_SHORT).show();
								check2 = 1;
								break;
	...
	public String setbLikeTeam()
	{
		...
		for( int i  = 0 ; i <= LoginInfoObject.getInstance().getLikeTeamList().size() ; i++)
		{
			...
			{
				likeTeam += LoginInfoObject.getInstance().getLikeTeamList().get(i) + ","+bestTeam.getTeamName();
				break;
			}
	 ...
	 public String setiLikeTeam()
		{
			...
			for( int i  = 0 ; i <= LoginInfoObject.getInstance().getLikeTeamList().size() ; i++)
			{
				...
				if( i == LoginInfoObject.getInstance().getLikeTeamList().size() -1 )
				{
					likeTeam += LoginInfoObject.getInstance().getLikeTeamList().get(i) + ","+issueTeam.getTeamName();
					break;
				}
				likeTeam += LoginInfoObject.getInstance().getLikeTeamList().get(i)+",";
			}
		...
		public String setbLikeMan()
		{
			...
			for( int i = 0 ; i <= bestTeam.getLikeMans().size() ; i++)
			{
				...
				{
					str += bestTeam.getLikeMans().get(i)+","+LoginInfoObject.getInstance().getId();
					break;
				}
		...
		public String setiLikeMan()
		{
			...
			for( int i = 0 ; i <= issueTeam.getLikeMans().size() ; i++)
			{
				...
				{
					str += issueTeam.getLikeMans().get(i)+","+LoginInfoObject.getInstance().getId();
					break;
				}
	    ...
	    public void requestTeamLikeUp(String name)
		{
			...
			loginParams.add(new Params("likecount", (bestTeam.getLikeCount() + 1)+""));
	    	...
	    	else
	    	{
				...
				loginParams.add(new Params("likecount", (issueTeam.getLikeCount() + 1)+""));
				loginParams.add(new Params("id", LoginInfoObject.getInstance().getId()));
			...
			try{
				...
				if(object.getJSONObject("result").optString("type").equals("ServiceType.MSG_TEAM_LIKE_UP"))
				{
					...
					if(result.equals("true"))
					{
						...
						if( checkbestissue )
						{
							LoginInfoObject.getInstance().getLikeTeamList().add(issueTeam.getTeamName());
							//로컬 카운트 + 1 , 그 팀 이 직접가지고 있는 라이크 눌른 인원들에 대한 아이디 추가
							for(int i = 0 ; i < BaseActivity.getTeamObject().size() ; i++)
							{
								if(issueTeam.getTeamName().equals(BaseActivity.getTeamObject().get(i).getTeamName()))
								{
						...
						{
							LoginInfoObject.getInstance().getLikeTeamList().add(bestTeam.getTeamName());
							//로컬 카운트 + 1 , 그 팀 이 직접가지고 있는 라이크 눌른 인원들에 대한 아이디 추가
							for(int i = 0 ; i < BaseActivity.getTeamObject().size() ; i++)
							{
								if(bestTeam.getTeamName().equals(BaseActivity.getTeamObject().get(i).getTeamName()))
								{
									BaseActivity.getTeamObject().get(i).setLikeCount(BaseActivity.getTeamObject().get(i).getLikeCount()+1);
									BaseActivity.getTeamObject().get(i).getLikeMans().add(LoginInfoObject.getInstance().getId());
									dataUiset();
